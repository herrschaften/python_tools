...
class TelegramArchiverGUI:
    ...
    def set_chat_status(self, status, items):
        ...
        self.thread_safe_update(self.log_message, f"🔧 Config updated: {included_count} included, {excluded_count} excluded")
    ...
    def should_include_chat(self, chat):
        """Check if chat should be included - FIXED LOGIC"""
        chat_id = str(chat['id'])
        chat_type = chat['type']
        
        # DEBUG: Log the decision process
        debug_msg = f"🔍 Checking {chat['title']} (ID: {chat_id}, Type: {chat_type})"
        
        # 1. EXPLICIT EXCLUSION takes priority
        if chat_id in self.config.get('excluded_chats', []):
            debug_msg += " → EXCLUDED (explicit)"
            self.thread_safe_update(self.log_message, debug_msg)
            return False
        
        # 2. EXPLICIT INCLUSION takes priority over type filters
        if chat_id in self.config.get('included_chats', []):
            debug_msg += " → INCLUDED (explicit)"
            self.thread_safe_update(self.log_message, debug_msg)
            return True
        ...
        if chat_type == 'private' and self.private_var.get():
            type_included = True
        elif chat_type in ['group', 'supergroup'] and self.group_var.get():
            type_included = True
        ...
        debug_msg += f" → {'INCLUDED' if type_included else 'EXCLUDED'} (type filter)"
        self.thread_safe_update(self.log_message, debug_msg)
    ...
    def start_archive(self):
        ...
        self.log_message("🔧 ARCHIVE CONFIGURATION:")
        self.log_message(f"   Private chats: {'✅' if self.private_var.get() else '❌'}")
        self.log_message(f"   Groups: {'✅' if self.group_var.get() else '❌'}")
        self.log_message(f"   Channels: {'✅' if self.channel_var.get() else '❌'}")
        self.log_message(f"   Bots: {'✅' if self.bot_var.get() else '❌'}")
        self.log_message(f"   Explicitly included: {len(self.config.get('included_chats', []))}")
        self.log_message(f"   Explicitly excluded: {len(self.config.get('excluded_chats', []))}")
        self.log_message("")
        
        # Check which chats will be processed
        included_chats = []
        self.log_message("🔍 CHECKING EACH CHAT:")
        ...
        self.log_message("")
        self.log_message(f"📊 FINAL RESULT: Will process {len(included_chats)} out of {len(self.chats)} total chats")
        
        if not included_chats:
            messagebox.showwarning("Warning", "No chats selected for archiving!")
            return
        
        # Show list of included chats for confirmation
        if len(included_chats) <= 10:
            self.log_message("📋 CHATS TO PROCESS:")
            for chat in included_chats:
                self.log_message(f"   • {chat['title']}")
        else:
            self.log_message(f"📋 CHATS TO PROCESS: {len(included_chats)} chats (too many to list)")
