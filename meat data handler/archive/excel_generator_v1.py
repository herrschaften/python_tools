            ...
            for row_idx, row_data in enumerate(data, 2):
                if row_data['thumbnail_path'] and os.path.exists(row_data['thumbnail_path']):
                    try:
                        ...
                        # Position image exactly within the cell boundaries
                        cell_ref = f'B{row_idx}'
                        
                        # Try different positioning approaches
                        try:
                            # Method 1: Use cell reference directly
                            ...
                            try:
                                ...
                                # Method 3: Pixel positioning (fallback)
                                from openpyxl.drawing.spreadsheet_drawing import AnchorMarker, TwoCellAnchor
                                
                                # Calculate exact cell coordinates
                                col_idx = 1  # Column B (0-indexed)
                                row_idx_0 = row_idx - 1  # 0-indexed row
                                
                                # Create precise anchor
                                start_marker = AnchorMarker(col=col_idx, row=row_idx_0, colOff=0, rowOff=0)
                                end_marker = AnchorMarker(col=col_idx, row=row_idx_0, colOff=cell_width_pixels*9525, rowOff=cell_height_pixels*9525)
                        ...
                        self.log_message(f"Could not add thumbnail for row {row_idx}: {str(thumb_error)}")
