...
class ImageExcelGenerator:
    ...
    def setup_ui(self):
        ...
        # Output info
        output_info = ttk.Label(main_frame, text="Excel file will be created in the parent folder of the selected images folder", 
    ...
    def get_subfolder_path(self, image_path):
        """Get the subfolder path including the selected folder and subfolders, up to 3 levels deep"""
        try:
            # Get the selected folder name
            selected_folder_name = os.path.basename(self.image_folder)
            ...
            # Limit to 3 subfolders maximum (not including the selected folder)
            if len(folder_parts) > 3:
                folder_parts = folder_parts[:3]
            
            # Construct the full path starting with the selected folder
            if folder_parts:
                # Include selected folder + subfolders
                full_path_parts = [selected_folder_name] + folder_parts
                subfolder_path = "/".join(full_path_parts) + "/"
            else:
                # Just the selected folder for root images
                subfolder_path = selected_folder_name + "/"
            
            return subfolder_path
            
        except Exception as e:
            self.log_message(f"Error getting subfolder path for {image_path}: {str(e)}")
            return ""
    ...
    def process_images(self):
        ...
        for i, image_path in enumerate(sorted_image_files):
            ...
            # Get subfolder path
            subfolder_path = self.get_subfolder_path(image_path)
            ...
            # Prepare row data
            row_data = {
                'subfolder_path': subfolder_path,  # Store for formula construction
            ...
            self.log_message(f"✓ Processed: {filename} (subfolder: {subfolder_path if subfolder_path else 'root'})")
    ...
    def create_excel_file(self, data):
        ...
        try:
            # Create output filename in parent directory of selected folder
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            output_filename = f"ImageIndex_{timestamp}.xlsx"
            
            # Get parent directory of the selected image folder
            ...
            self.log_message(f"Parent directory: {parent_directory}")
            self.log_message(f"Selected folder: {self.image_folder}")
            ...
            # Define headers - updated with new column name
            headers = ["'@img", 'thumbnail', 'name', 'prefix', 'fig_number', 'sub_number', 
            ...
            for row_idx, row_data in enumerate(data, 2):
                try:
                    # Column A: '@img formula - "folder/subfolder/"&C2
                    img_formula = f'"{row_data["subfolder_path"]}"&C{row_idx}'
                    ws.cell(row=row_idx, column=1, value=f'={img_formula}')
            ...
            column_widths = [25, 15, 25, 10, 12, 12, 30, 20, 40, 25, 60]  # Increased first column width
    ...
    def preview_data(self):
        ...
        for i, image_path in enumerate(image_files[:10]):  # Preview first 10
            ...
            self.log_message(f"  Subfolder: {subfolder_path if subfolder_path else '(root folder)'}")
            self.log_message(f"  @img formula result: {subfolder_path}{filename}")
