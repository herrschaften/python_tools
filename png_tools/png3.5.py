        ...
        for i, file_path in enumerate(self.file_paths):
            try:
                ...
                if self.custom_palette:
                    ...
                    img_indexed = img.quantize(colors=len(self.custom_palette), palette=palette_img, dither=dither_value)
                else:
                    # FIXED: Implement the two-step process for non-custom palette
                    # Step 1: Generate a palette without dithering
                    print("No custom palette provided. Generating a palette without dithering first...")
                    ...
                    img_indexed = img.quantize(colors=len(unique_indices), palette=new_palette_img, dither=dither_value)
...
class IndexedColorConverter(QMainWindow):
    ...
    def setup_single_image_tab(self):
        ...
        downscale_method_layout = QHBoxLayout()
        downscale_method_layout.addWidget(QLabel("Downscale Method:"))
        self.downscale_method_combo = QComboBox()
        self.downscale_method_combo.addItems(["LANCZOS", "BICUBIC", "BILINEAR", "NEAREST"])
        downscale_method_layout.addWidget(self.downscale_method_combo)
        left_panel.addLayout(downscale_method_layout)
        ...
        upscale_method_layout = QHBoxLayout()
        upscale_method_layout.addWidget(QLabel("Upscale Method:"))
        self.upscale_method_combo = QComboBox()
        self.upscale_method_combo.addItems(["NEAREST", "BILINEAR", "BICUBIC"])
        upscale_method_layout.addWidget(self.upscale_method_combo)
        left_panel.addLayout(upscale_method_layout)
    ...
    def setup_batch_tab(self):
        ...
        # Dithering option
        self.batch_dithering_checkbox = QCheckBox("Use Diffusion Dithering (color reduction)")
        self.batch_dithering_checkbox.setChecked(True)
        settings_layout.addWidget(self.batch_dithering_checkbox, 1, 0, 1, 2)
        
        # Upscale dithering option (new!)
        self.batch_upscale_dithering_checkbox = QCheckBox("Apply Dithering During Upscale")
        self.batch_upscale_dithering_checkbox.setChecked(False)
        settings_layout.addWidget(self.batch_upscale_dithering_checkbox, 2, 0, 1, 2)
        
        # Number of colors
        settings_layout.addWidget(QLabel("Number of Colors:"), 3, 0)
        self.batch_num_colors_spin = QSpinBox()
        self.batch_num_colors_spin.setRange(2, 256)
        self.batch_num_colors_spin.setValue(16)
        settings_layout.addWidget(self.batch_num_colors_spin, 3, 1)
        
        # Max dimension (longest side) for downscale
        settings_layout.addWidget(QLabel("Downscale Longest Side (px):"), 4, 0)
        ...
        settings_layout.addWidget(self.max_dimension_spin, 4, 1)
        
        # Fixed length dimension
        settings_layout.addWidget(QLabel("Fixed Length (px):"), 5, 0)
        ...
        settings_layout.addWidget(self.max_length_spin, 5, 1)
        
        # Downscale method dropdown
        settings_layout.addWidget(QLabel("Downscale Method:"), 6, 0)
        self.batch_downscale_method_combo = QComboBox()
        self.batch_downscale_method_combo.addItems(["LANCZOS", "BICUBIC", "BILINEAR", "NEAREST"])
        settings_layout.addWidget(self.batch_downscale_method_combo, 6, 1)
        
        # Upscale size
        settings_layout.addWidget(QLabel("Upscale Longest Side (px):"), 7, 0)
        ...
        settings_layout.addWidget(self.batch_upscale_spin, 7, 1)
        
        # Upscale method dropdown
        settings_layout.addWidget(QLabel("Upscale Method:"), 8, 0)
        self.batch_upscale_method_combo = QComboBox()
        self.batch_upscale_method_combo.addItems(["NEAREST", "BILINEAR", "BICUBIC"])
        settings_layout.addWidget(self.batch_upscale_method_combo, 8, 1)
    ...
    def process_batch(self):
        ...
        # Get upscale and downscale methods
        upscale_method = self.batch_upscale_method_combo.currentText()
        downscale_method = self.batch_downscale_method_combo.currentText()
        ...
        # Get dithering states
        dithering_state = self.batch_dithering_checkbox.isChecked()
        upscale_dithering = self.batch_upscale_dithering_checkbox.isChecked()
        
        print(f"Batch processing - Dithering: {dithering_state}, Custom Palette: {self.use_custom_palette_checkbox.isChecked()}")
