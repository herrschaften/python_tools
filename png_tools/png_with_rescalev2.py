...
class ImageProcessor(QThread):
    ...
    def __init__(self, file_paths, num_colors, max_size=None, max_length=None, output_folder=None, 
        ...
        # Store dithering as boolean
        self.use_dithering = use_dithering
        self.upscale_size = upscale_size
        print(f"ImageProcessor initialized with dithering: {self.use_dithering}")
        ...
        for i, file_path in enumerate(self.file_paths):
            try:
                ...
                if self.custom_palette:
                    ...
                    # CRITICAL FIX: Use direct integer values (1 or 0) for dithering parameter
                    print(f"Applying quantize with custom palette, dither={self.use_dithering}")
                    img_indexed = img.quantize(colors=len(self.custom_palette), palette=palette_img, dither=self.use_dithering)
                else:
                    # CRITICAL FIX: Use direct integer values (1 or 0) for dithering parameter
                    print(f"Applying quantize without custom palette, dither={self.use_dithering}")
                    img_indexed = img.quantize(colors=self.num_colors, dither=self.use_dithering)
                
                # Upscale with nearest neighbor if specified
                # FIX: Don't apply dithering during upscaling
                if self.upscale_size:
                    ...
                    img_indexed = img_indexed.resize((upscale_width, upscale_height), PILImage.NEAREST)
...
class ColorEditorThread(QThread):
    ...
    def __init__(self, input_path, output_path, color_mapping, preview_only=False, use_dithering=True, upscale_size=None):
        ...
        # CRITICAL FIX: Store dithering as an integer value (1 or 0)
        self.use_dithering = 1 if use_dithering else 0  # Convert boolean to integer 1 or 0
        self.upscale_size = upscale_size
        print(f"ColorEditorThread initialized with dithering: {self.use_dithering}")
    ...
    def run(self):
        try:
            ...
            if img.mode != 'P':
                ...
                # FIX: We don't need to apply dithering here as we're just doing a direct conversion
            ...
            if self.upscale_size:
                ...
                # Upscale using nearest neighbor for that pixelated look
                new_img = new_img.resize((upscale_width, upscale_height), PILImage.NEAREST)
...
class IndexedColorConverter(QMainWindow):
    ...
    def convert_single_image(self):
        ...
        # CRITICAL FIX: Get dithering state directly from the checkbox
        # This is the key fix for ensuring dithering is applied when the convert button is pressed
        use_dithering = self.dithering_checkbox.isChecked()
        print(f"Single image conversion - Dithering checkbox state: {use_dithering}")
        ...
        self.processor = ImageProcessor(
            ...
            use_dithering=use_dithering,  # Directly pass the checkbox state
    ...
    def preview_changes(self):
        ...
        # FIX: Use the ColorEditorThread to generate the preview
        ...
        # CRITICAL FIX: Print the current state of dithering checkbox
        dithering_state = self.dithering_checkbox.isChecked()
        print(f"Preview changes - Dithering checkbox state: {dithering_state}")
        
        try:
            # Setup thread for preview only
            self.color_editor = ColorEditorThread(
                self.current_indexed_image_path, 
                "", # No output path needed for preview
                color_mapping,
                preview_only=True,
                use_dithering=self.dithering_checkbox.isChecked(),  # Use direct checkbox state
                upscale_size=upscale_size
            )
    ...
    def render_with_new_colors(self, preview_only=False):
        ...
        # CRITICAL FIX: Print the current state of dithering checkbox
        dithering_state = self.dithering_checkbox.isChecked()
        print(f"Render with new colors - Dithering checkbox state: {dithering_state}")
    ...
    def process_batch(self):
        ...
        # CRITICAL FIX: Print the current state of batch dithering checkbox
        dithering_state = self.batch_dithering_checkbox.isChecked()
        print(f"Batch processing - Dithering checkbox state: {dithering_state}")
        print(f"Batch processing - Using custom palette: {self.use_custom_palette_checkbox.isChecked()}")
        ...
        self.batch_processor = ImageProcessor(
            ...
            use_dithering=self.batch_dithering_checkbox.isChecked(),  # Use direct checkbox state
            upscale_size=upscale_size
        )
